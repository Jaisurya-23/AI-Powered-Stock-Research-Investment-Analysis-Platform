...
def _json_default(obj):
    """
    Fallback serializer for json.dumps() covering types that show up in the
    yfinance/pandas data (Timestamp, numpy scalars, date/datetime, sets,
    frozendict-wrapped values, etc.) so saving a report to history never
    crashes on an unexpected type.
    """
    # pandas Timestamp / numpy datetime64 (avoid a hard pandas import here;
    # duck-type via isoformat which both support)
    if hasattr(obj, "isoformat"):
        try:
            return obj.isoformat()
        except Exception:
            pass
    # numpy scalar types (int64, float64, bool_, etc.) all support .item()
    if hasattr(obj, "item"):
        try:
            return obj.item()
        except Exception:
    ...
    # Last resort: stringify rather than crash the whole request
    return str(obj)
...
def simple_markdown(text):
    """Lightweight markdown -> HTML for the AI analysis text (bold, headers, bullets, paragraphs)."""
    if not text:
        return ""
    import html as html_module
    text = html_module.escape(text)
    text = re.sub(r'\*\*(.+?)\*\*', r'<strong>\1</strong>', text)
    text = re.sub(r'^### (.+)$', r'<h6 class="mt-3 mb-2">\1</h6>', text, flags=re.MULTILINE)
    text = re.sub(r'^## (.+)$', r'<h5 class="mt-3 mb-2">\1</h5>', text, flags=re.MULTILINE)
    text = re.sub(r'^# (.+)$', r'<h4 class="mt-3 mb-2">\1</h4>', text, flags=re.MULTILINE)
    text = re.sub(r'^[\*\-]\s+(.+)$', r'<li>\1</li>', text, flags=re.MULTILINE)
    text = re.sub(r'(<li>.*?</li>\n?)+', lambda m: '<ul class="mb-3">' + m.group(0) + '</ul>', text, flags=re.DOTALL)
...
def ensure_schema():
    """Add columns for structured report data if they don't exist yet (safe to run every startup)."""
    try:
        conn = get_db_connection()
        existing_cols = {row["name"] for row in conn.execute("PRAGMA table_info(research_history)")}
        new_cols = {
            "financial_json": "TEXT",
            "news_json": "TEXT",
            "checklist_json": "TEXT",
        ...
        for col, col_type in new_cols.items():
            if col not in existing_cols:
                conn.execute(f"ALTER TABLE research_history ADD COLUMN {col} {col_type}")
        conn.commit()
        conn.close()
    except Exception as e:
        print(f"⚠️ Could not migrate research_history schema: {e}")
    ...
    try:
        ...
        conn.execute(
            """INSERT INTO research_history
               (company, report, created_at, financial_json, news_json, checklist_json, ai_report)
               VALUES (?, ?, ?, ?, ?, ?, ?)""",
            (
                enhanced_financial['data']['basic_info']['company_name'],
                final_report,
                created_at,
                safe_json_dumps(enhanced_financial),
                safe_json_dumps(news_analysis),
                safe_json_dumps(checklist_results),
                ai_report,
            )
        )
        conn.commit()
...
def view_report(report_id):
    ...
    ai_report = None
    try:
        if "financial_json" in row_keys and row["financial_json"]:
            financial_data = json.loads(row["financial_json"])
        if "news_json" in row_keys and row["news_json"]:
            news_data = json.loads(row["news_json"])
        if "checklist_json" in row_keys and row["checklist_json"]:
            checklist_data = json.loads(row["checklist_json"])
        if "ai_report" in row_keys:
            ai_report = row["ai_report"]
    except Exception as e:
        print(f"⚠️ Could not parse stored structured data for report {report_id}: {e}")
