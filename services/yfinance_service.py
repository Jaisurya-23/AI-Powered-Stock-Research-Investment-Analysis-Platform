...
# --- Yahoo's edge now blocks requests based on TLS fingerprint, not just
# request rate. Python's stock `requests`/urllib3 handshake gets an
# instant 429 ("crumb=Edge: Too Many Requests") regardless of pacing.
# curl_cffi impersonates a real browser's TLS fingerprint, which is the
# standard workaround yfinance itself now recommends. Fall back to a
# plain requests.Session if curl_cffi isn't installed, so the app still
# runs (just without the fingerprint fix) rather than crashing on import.
try:
    from curl_cffi import requests as curl_requests
    _SHARED_SESSION = curl_requests.Session(impersonate="chrome")
except ImportError:
    print(
        "Warning: curl_cffi not installed - Yahoo Finance requests may be "
        "blocked at the edge. Install with: pip install curl_cffi"
    )
    _SHARED_SESSION = requests.Session()

# --- Simple in-process cache so repeat lookups (e.g. resolve + fetch for
# the same ticker in one request) don't re-hit Yahoo at all. ---
_INFO_CACHE = {}
_CACHE_TTL_SECONDS = 300  # 5 minutes


def _get_info_with_retry(ticker_symbol: str, max_retries: int = 3):
    """
    Fetch yf.Ticker(...).info with caching + exponential backoff on 429s.
    Returns {} if the ticker has no usable data after retries.
    """
    now = time.time()
    cached = _INFO_CACHE.get(ticker_symbol)
    ...
    for attempt in range(max_retries):
        try:
            stock = yf.Ticker(ticker_symbol, session=_SHARED_SESSION)
            info = stock.info or {}
            # yfinance sometimes returns a near-empty dict on soft failures
            # instead of raising - treat that as a signal to retry too.
            if info and (info.get("regularMarketPrice") is not None or info.get("longName")):
                _INFO_CACHE[ticker_symbol] = (now, info)
                return info
            last_err = ValueError("Empty/incomplete info payload")
        except Exception as e:
            last_err = e
            msg = str(e)
            if "429" in msg or "Too Many Requests" in msg:
                # Exponential backoff with jitter: ~1.5s, 3s, 6s...
                sleep_for = (1.5 * (2 ** attempt)) + random.uniform(0, 0.5)
                time.sleep(sleep_for)
                continue
            else:
                # Non-rate-limit error - no point retrying identically
                break

    print(f"Warning: could not fetch info for {ticker_symbol}: {last_err}")
    # Cache the failure briefly too, so a bad ticker doesn't get hammered
    # again a second later within the same request cycle.
    _INFO_CACHE[ticker_symbol] = (now, {})
    return {}
...
class EnhancedFinancialAnalyzer:
    ...
    def __init__(self, ticker: str):
        ...
        self.stock = yf.Ticker(self.ticker, session=_SHARED_SESSION)
        self.info = _get_info_with_retry(self.ticker)
        if not self.info:
            raise ValueError(
                f"No data available for '{self.ticker}' (Yahoo Finance returned no data, "
                f"possibly due to rate limiting or an invalid ticker)."
            )
    ...
    def _resolve_ticker(self, ticker: str):
        ...
        # NOTE: We deliberately do NOT probe Yahoo here to "validate" the
        # ticker. Every extra .info call is an extra request against
        # Yahoo's throttled endpoint, and __init__ is about to fetch this
        # exact ticker's info anyway. If the plain ticker turns out to be
        # wrong, get_comprehensive_data()/the caller can catch the
        # resulting error and the user can retry with an explicit suffix
        # (e.g. "TICKER.NS"). This turns 3 Yahoo requests per lookup into 1.
        return ticker
